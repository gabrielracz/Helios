...
void init_hash_state() {
	memcpy(hash, hash_init, sizeof(uint32_t)*8);
}
	/*_________operations________ */
uint32_t SHR(uint32_t num, int shift){
	return num >> shift;
}

uint32_t ROTR(uint32_t num, int shift){
...
void process_input(uint8_t* buffer, size_t buflen, uint32_t* message, size_t msglen){
	...
	for(size_t i = 0; i < buflen; i++){
		...
		switch(cycle) {
			case 0:
				...
				break;
			case 1:
				elem = (uint32_t)(buffer[i]) << 24;
				break;
			case 2:
				elem += (uint32_t)(buffer[i]) << 16;
				break;
			case 3:
				elem += (uint32_t)(buffer[i]) << 8;
				break;
		}
	}
	...
	message[msglen - 1] = (uint32_t)buflen*8;
}
...
int sha256(uint8_t* buffer, size_t buflen, char* output){
  ...
  uint32_t message[16] = {0};
	process_input(buffer, buflen, message, 16);
...
void loop() {
  ...
  double delta = (double)(end - start)/1e6f;
  unsigned long hash_rate = iterations / delta;
  char serial_str[1024];
  // sprintf(serial_str, "%lu hash/sec    %s -> %s", hash_rate, input, output);
  Serial.print(hash_rate);
  Serial.print(" hash/sec      ");
  Serial.print(input);
  Serial.print(" -> ");
